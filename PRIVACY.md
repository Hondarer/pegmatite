...
PlantUML In-Place Preview renders PlantUML source code found in supported web pages as diagram images.
It is a maintained fork of Pegmatite; the repository keeps the name `pegmatite` for continuity with the upstream project.
...
PlantUML In-Place Preview renders diagrams entirely inside the browser.
...
PlantUML In-Place Preview stores no user data; it no longer uses the Chrome storage API.
...
PlantUML In-Place Preview は、対応する Web ページ内にある PlantUML のソースコードを図として描画します。
Pegmatite のフォークを保守したもので、リポジトリ名 `pegmatite` はフォーク元との一貫性のため残しています。
...
PlantUML In-Place Preview は、すべてブラウザ内で描画します。
...
この動作と、それを設定していた Base URL の項目は現バージョンでは存在しません。

### 収集、保持、利用

拡張機能の開発者は、PlantUML のソースや閲覧履歴を収集、保持、販売、利用することはありません。
これは広告、分析、与信判断、および図の描画に関係しないあらゆる目的にあてはまります。
PlantUML In-Place Preview は利用者のデータを一切保存せず、Chrome の storage API も使用しません。
